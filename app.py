...
st.set_page_config(
    page_title="SafeGuard | Cyber Security Dashboard",
    page_icon="🛡️",
    layout="wide",
...
def create_pdf_report(text, sender, prediction, confidence, urls, keywords, ips):
    ...
    pdf.cell(200, 10, txt="SafeGuard Security Report", ln=True, align='C')
    pdf.ln(10)
    ...
    pdf.cell(200, 10, txt=f"Sender Identity: {sender if sender else 'Unknown'}", ln=True)
...
with st.sidebar:    
    # --- LIVE DASHBOARD (Moved to Sidebar) ---
    st.markdown("### 📊 Live Dashboard")
    
    # Custom HTML for Dashboard Metrics in Sidebar
    st.markdown(f"""
    <div class="dash-card">
...
# --- UPDATED SUBTITLE ---
st.markdown("### Phishing Email Analysis System")
